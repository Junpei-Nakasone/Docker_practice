...
# docker commitコマンド  
```
$docker commit <コンテナ名かID> <イメージ名>:<タグ名>
```
上記コマンドを実行することでコンテナの状態をイメージとして保存できる
注意するのは、docker commitでイメージを作成した場合、コンテナで行われた作業はどこにも記録が残らないこと。
通常dockerイメージを作る際にdockerファイルで実行された各コマンドはイメージの各レイヤーとして何のコマンドが実行されたかが記録されている。


# apk
Alpine Linuxのパッケージ管理コマンド。
AlpinehはLinuxディストリビューションの一つで軽量。
